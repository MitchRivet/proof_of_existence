...
contract("Proofs", function(accounts) {
  ...
  it("should retrieve the number of proofs", function() {
    eturn Proofs.deployed().then(function(instance) {
        var proofsInstance = instance;
        let promiseArray = [];
        for (var i =0; i < 3; i++) {
            promiseArray.push(proofsInstance.createProof("test " + i , "a test proof", "image hash"))
        }
          ...
          .then(function(res) {
              assert.equal(res.toNumber(), 3, "counted 3 created proofs")
