...
class App extends Component {
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
      proofs
        ...
        .then(instance => {
          ...
          return proofsInstance.getProofIds.call({ from: accounts[0] });
        })
        .then(result => {
  ...
  createProof(form) {
    ...
    return this.state.proofsInstance
      ...
      .then(result => {
        debugger;
        let finishedProof = this.state.proofs.filter(p => p.id === this.state.numberOfProofs);
        let updatedProofs = [...this.state.proofs, finishedProof]
        //this.setState({});
      });
  }
