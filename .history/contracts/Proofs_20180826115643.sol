...
contract Proofs {
    ...
    function getProof(uint id) returns (Proof) {
        return proofs[id];
    }
    ...
    function createProof(string title, string description, string image, string[] tags) public  {
        proofCount += 1; 
        proofs[proofCount] = Proof({id: proofCount, creator: owner, title: title, description: description, image: image, tags: tags });
